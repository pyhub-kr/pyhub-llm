...
def test_ask_with_single_file():
    ...
    try:
        with patch("pyhub.llm.LLM.create") as mock_create:
            ...
            result = runner.invoke(app, ["ask", "분석해주세요", "--file", str(temp_file), "--no-stream"])
    ...
    try:
        with patch("pyhub.llm.LLM.create") as mock_create:
            ...
            result = runner.invoke(
                app, ["ask", "분석해주세요", "--file", str(temp_file1), "--file", str(temp_file2), "--no-stream"]
            )
...
def test_ask_with_nonexistent_file():
    ...
    result = runner.invoke(app, ["ask", "분석해주세요", "--file", "nonexistent_file.py"])
...
def test_ask_with_file_and_context():
    ...
    try:
        with patch("pyhub.llm.LLM.create") as mock_create:
            ...
            result = runner.invoke(
                app, ["ask", "분석해주세요", "--file", str(temp_file), "--context", "추가 컨텍스트", "--no-stream"]
            )
