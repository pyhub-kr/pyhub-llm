...
class TestPromptTemplate:
    ...
    def test_partial_prompt(self):
        ...
        prompt = PromptTemplate(template="Hello {name}, you are {age} years old.", input_variables=["name", "age"])
    ...
    def test_save_and_load(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            prompt = PromptTemplate(template="Test {variable}", input_variables=["variable"], metadata={"test": True})
    ...
    def test_jinja2_format(self):
        ...
        prompt = PromptTemplate(template="Hello {{ name }}!", input_variables=["name"], template_format="jinja2")
        result = prompt.format(name="Charlie")
        assert result == "Hello Charlie!"

    def test_unsupported_format(self):
        """Test unsupported template format."""
        prompt = PromptTemplate(template="Hello {{name}}!", input_variables=["name"], template_format="mustache")
...
class TestPromptHub:
    ...
    def test_push_and_pull_custom_prompt(self):
        ...
        custom_prompt = PromptTemplate(
            template="Custom {test} prompt", input_variables=["test"], metadata={"custom": True}
        )
    ...
    def test_custom_cache_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Push a prompt
            prompt = PromptTemplate(template="Test {var}", input_variables=["var"])
