...
class TestMCPConnectionPolicy:
    ...
    async def test_optional_policy_default(self):
        ...
        llm = LLM.create(
            "gpt-4o-mini", mcp_servers=[{"type": "stdio", "name": "test_server", "cmd": ["nonexistent_command_xyz"]}]
        )
    ...
    async def test_required_policy_failure(self):
        ...
        llm = LLM.create(
            "gpt-4o-mini",
            mcp_servers=[{"type": "stdio", "name": "test_server", "cmd": ["nonexistent_command_xyz"]}],
    ...
    async def test_required_policy_with_echo(self):
        ...
        llm = LLM.create(
            "gpt-4o-mini",
            mcp_servers=[{"type": "stdio", "name": "echo_server", "cmd": ["echo", "hello"]}],  # 간단한 echo 명령
            mcp_policy=MCPConnectionPolicy.REQUIRED,
        )
    ...
    async def test_policy_with_multiple_servers(self):
        ...
        # WARN 정책
        llm = LLM.create("gpt-4o-mini", mcp_servers=servers, mcp_policy=MCPConnectionPolicy.WARN)
