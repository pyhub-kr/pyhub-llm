            ...
            for attempt in range(strategy.max_retries + 1):
                try:
                    ...
                    if attempt < strategy.max_retries:
                        ...
                        logger.info(f"Retrying {func.__name__} in {delay:.2f}s (attempt {attempt + 1}/{strategy.max_retries})")
                        time.sleep(delay)
                    else:
...
def example_error_handler_setup():
    ...
    error_handler.register_callback("AuthenticationError", on_auth_error)
