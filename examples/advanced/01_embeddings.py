...
def example_basic_embedding():
    ...
    print(f"임베딩 차원: {len(embedding.embeddings[0])}")
    print(f"임베딩 벡터 (처음 5개): {embedding.embeddings[0][:5]}")
    ...
    for i, text in enumerate(texts):
        print(f"  {i+1}. {text[:30]}... → 벡터 크기: {len(embeddings.embeddings[i])}")
...
def example_document_search():
    ...
    class DocumentSearch:
        ...
        def add_documents(self, docs: List[Dict[str, Any]]):
            ...
            new_embeddings = self.llm.embed(texts).embeddings
    ...
    results = search_engine.search_with_filter(
        query,
        filter_fn=lambda doc: doc['category'] == 'AI',
        top_k=2
    )
...
def example_clustering():
    ...
    # 임베딩 생성
    print("임베딩 생성 중...")
    embeddings = llm.embed(documents).embeddings
...
def example_semantic_search_rag():
    ...
    class RAGSystem:
        ...
        def add_knowledge(self, documents: List[Dict[str, str]]):
            ...
            new_embeddings = self.embed_llm.embed(texts).embeddings
        ...
        def retrieve(self, query: str, top_k: int = 3) -> List[Dict]:
            """관련 문서 검색"""
            query_embedding = self.embed_llm.embed(query).embeddings[0]
