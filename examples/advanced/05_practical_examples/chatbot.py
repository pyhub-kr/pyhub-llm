...
from pyhub.llm.types import Message, ToolCall
...
class AdvancedChatbot:
    ...
    def __init__(self, model: str = "gpt-4o-mini"):
        ...
        self.cache = MemoryCache(ttl=3600)  # 1시간 캐시
...
필요한 경우 도구를 사용하여 더 정확한 정보를 제공합니다."""
    ...
    def _execute_tool(self, tool_call: ToolCall, conversation_id: str) -> str:
        """도구 실행"""
        if tool_call.name == "get_weather":
            location = tool_call.parameters.get("location", "서울")
            # 실제 구현에서는 날씨 API 호출
        ...
        elif tool_call.name == "remember_preference":
            key = tool_call.parameters.get("key", "")
            ...
            if use_tools:
                ...
                if hasattr(reply, 'tool_calls') and reply.tool_calls:
                    for tool_call in reply.tool_calls:
                        result = self._execute_tool(tool_call, conversation_id)
                        tool_results.append({
                            "tool": tool_call.name,
                            "result": result
                        })
