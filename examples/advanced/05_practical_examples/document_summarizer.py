...
# from pyhub.llm.templates import TemplateEngine  # 템플릿 엔진 대신 직접 문자열 포맷팅 사용
...
class DocumentSummarizer:
    ...
    def __init__(self, model: str = "gpt-4o-mini"):
        ...
        # self.cache = FileCache("summaries")  # 캐시 기능은 현재 예제에서 사용하지 않음
...
4. 키워드: (쉼표로 구분)""",
        ...
        if doc_type == DocumentType.MARKDOWN:
            ...
            for line in content.split("\n"):
                ...
                if match:
                    ...
                    current_section = DocumentSection(
                        title=title, content="", level=level, word_count=0, position=position
                    )
                    current_content = []
                    position += 1
                else:
            ...
            for i, para in enumerate(paragraphs):
                if para.strip():
                    sections.append(
                        DocumentSection(
                            title=f"섹션 {i+1}", content=para.strip(), level=1, word_count=len(para.split()), position=i
                        )
                    )
    ...
    def generate_executive_summary(self, sections: List[DocumentSection], max_words: int = 300) -> str:
        """전체 요약 생성"""
        # 섹션 요약들을 합침
        section_summaries = []
    ...
    def analyze_document_structure(self, sections: List[DocumentSection]) -> Dict[str, Any]:
        ...
        for section in sections:
            if section.level not in section_stats:
                section_stats[section.level] = {"count": 0, "total_words": 0, "avg_words": 0}
...
추천사항 (각각 한 문장):**"""
    ...
    def summarize(self, file_path: str, style: str = "detailed", include_recommendations: bool = True) -> SummaryResult:
        """문서 요약 메인 함수"""
