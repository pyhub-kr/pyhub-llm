...
class AdvancedTranslator:
    ...
    def __init__(self, model: str = "gpt-4o"):
        ...
        self.cache = FileCache("translations")
...
3. Translation notes (cultural/contextual considerations)"""
        ...
        self.template_engine.add_template(
            "detect_language",
            """Detect the language of the following text and respond with the ISO 639-1 language code (e.g., 'en', 'ko', 'ja'):
...
Language code:"""
    ...
    def translate(
        ...
        if not context:
            context = TranslationContext(
                domain=TranslationType.GENERAL,
                formality="neutral",
        ...
        prompt = self.template_engine.render_template(
            ...
            previous=context.previous_translations[-3:] if context.previous_translations else []
        )
        ...
        # 결과 파싱
        result = self._parse_translation_result(
            reply.text,
            text,
            source_language,
            target_language
    ...
    def _parse_translation_result(
        ...
        # 단어 수 계산
        word_count = {
            "source": len(original_text.split()),
            "target": len(main_translation.split())
        }
