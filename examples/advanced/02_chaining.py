    ...
    class ConditionalChain:
        ...
        def __init__(self):
            ...
            self.classifier = LLM.create(
                "gpt-4o-mini",
                system_prompt="텍스트의 주제를 '기술', '비즈니스', '일반' 중 하나로]만 답하세요."
            )
            
            # 주제별 전문 LLM
            self.tech_expert = LLM.create(
                "gpt-4o-mini",
                system_prompt="당신은 기술 전문가입니다. 기술적이고 상세한 답변을 제공하세요."
            )
            
            self.business_expert = LLM.create(
                "gpt-4o-mini",
                system_prompt="당신은 비즈니스 전문가입니다. 비즈니스 관점에서 답변하세요."
            )
            
            self.general_expert = LLM.create(
                "gpt-4o-mini",
                system_prompt="당신은 친절한 어시스턴트입니다. 쉽고 친근하게 설명하세요."
            )
    ...
    class DataProcessingPipeline:
        ...
        def __init__(self):
            ...
            self.extractor = LLM.create(
                "gpt-4o-mini",
                system_prompt="텍스트에서 주요 정보를 추출하여 JSON 형식으로 출력하세요."
            )
            
            self.validator = LLM.create(
                "gpt-4o-mini",
                system_prompt="데이터의 정확성을 검증하고 문제점을 지적하세요."
            )
            
            self.enricher = LLM.create(
                "gpt-4o-mini",
                system_prompt="주어진 정보를 보강하고 추가 컨텍스트를 제공하세요."
            )
            
            self.formatter = LLM.create(
                "gpt-4o-mini",
                system_prompt="정보를 보기 좋게 포맷팅하여 최종 보고서를 작성하세요."
            )
...
def example_feedback_loop_chain():
    ...
    class FeedbackChain:
        ...
        def __init__(self, max_iterations: int = 3):
            ...
            self.generator = LLM.create(
                "gpt-4o-mini",
                system_prompt="주어진 주제에 대한 설명을 작성하세요."
            )
            self.critic = LLM.create(
                "gpt-4o-mini",
                system_prompt="텍스트를 평가하고 구체적인 개선점을 제시하세요."
            )
            self.improver = LLM.create(
                "gpt-4o-mini",
                system_prompt="피드백을 바탕으로 텍스트를 개선하세요."
            )
