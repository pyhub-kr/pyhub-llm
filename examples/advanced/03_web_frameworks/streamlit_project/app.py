...
def save_message(role: str, content: str, tokens: int = 0):
    """메시지 저장"""
    message = {"role": role, "content": content, "timestamp": datetime.now().isoformat(), "tokens": tokens}
...
def main():
    ...
    with st.sidebar:
        ...
        with st.expander("고급 설정", expanded=False):
            ...
            system_prompt = st.text_area(
                "시스템 프롬프트", value="당신은 도움이 되고 친절한 AI 어시스턴트입니다.", height=100
            )
        ...
        if st.button("설정 적용", type="primary"):
            try:
                st.session_state.llm = LLM.create(
                    model, temperature=temperature, max_tokens=max_tokens, system_prompt=system_prompt
                )
        ...
        if st.session_state.conversations:
            ...
            for conv_id, conv in sorted(
                st.session_state.conversations.items(), key=lambda x: x[1]["updated_at"], reverse=True
            ):
                col1, col2 = st.columns([3, 1])
                with col1:
                    if st.button(f"📝 {conv['title'][:20]}...", key=f"conv_{conv_id}", use_container_width=True):
                        st.session_state.current_conversation_id = conv_id
    ...
    with tab2:
        ...
        with col2:
            analysis_options = st.multiselect(
                "분석 옵션", ["감정 분석", "요약", "키워드 추출", "개체명 인식"], default=["감정 분석", "요약"]
            )
                    ...
                    with st.spinner("분석 중..."):
                        ...
                        if "감정 분석" in analysis_options:
                            reply = st.session_state.llm.ask(
                                f"다음 텍스트의 감정을 분석하세요: {analysis_text}", choices=["긍정", "부정", "중립"]
                            )
                            results["감정"] = reply.choice

                        if "요약" in analysis_options:
                            reply = st.session_state.llm.ask(f"다음 텍스트를 한 문장으로 요약하세요: {analysis_text}")
    ...
    with tab3:
        ...
        if uploaded_file is not None:
            ...
            with col2:
                image_question = st.text_area("질문을 입력하세요", value="이 이미지를 자세히 설명해주세요.", height=100)
    ...
    with tab4:
        ...
        tool_option = st.selectbox("도구 선택", ["번역기", "코드 생성", "SQL 쿼리 생성", "정규식 생성"])
        ...
        if tool_option == "번역기":
            ...
            with col1:
                ...
                _ = st.selectbox("원문 언어", ["자동 감지", "한국어", "영어", "일본어", "중국어"])  # 현재 사용하지 않음
            ...
            with col1:
                code_description = st.text_area(
                    "코드 설명", placeholder="생성하고 싶은 코드를 설명하세요...", height=150
                )
            ...
            query_description = st.text_input("쿼리 설명", placeholder="예: 2024년에 가장 많이 주문한 사용자 top 10")
...
"""
            ...
            regex_description = st.text_input("정규식 설명", placeholder="예: 한국 전화번호 형식 (010-1234-5678)")

            test_strings = st.text_area(
                "테스트 문자열 (줄 단위)", placeholder="테스트할 문자열을 입력하세요...", height=100
            )
