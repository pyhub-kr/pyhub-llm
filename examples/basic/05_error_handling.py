...
def example_basic_error_handling():
    ...
    try:
        ...
        # OpenAI의 경우 AuthenticationError가 발생할 수 있음
    ...
    try:
        ...
        reply = llm.ask(very_long_text)  # noqa
    except Exception as e:
        # OpenAI의 경우 InvalidRequestError가 발생할 수 있음
        if "invalid" in str(e).lower() or "token" in str(e).lower() or "limit" in str(e).lower():
            print("❌ 잘못된 요청: 입력이 너무 길거나 형식이 잘못되었습니다.")
            print(f"   에러 메시지: {str(e)[:100]}...")
        else:
...
def example_timeout_handling():
    ...
    # 짧은 타임아웃 테스트
    result = ask_with_timeout(llm, "짧은 답변: 1+1은?", timeout_seconds=10)
