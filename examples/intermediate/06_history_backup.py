...
def example_sqlite_backup():
    ...
    # 여러 대화 세션 시뮬레이션
    sessions = [
        {"id": "tech_discussion", "messages": ["파이썬과 자바의 차이점은?", "웹 개발에는 어떤 언어가 좋을까요?"]},
        {"id": "ai_learning", "messages": ["머신러닝을 시작하려면?", "딥러닝과 머신러닝의 차이는?"]},
    ]
...
def example_advanced_backup():
    ...
    class AdvancedHistoryBackup(InMemoryHistoryBackup):
            ...
            for conv_id, messages in self.storage.items():
                for msg in messages:
                    if keyword.lower() in msg.content.lower():
                        results.append(
                            {"conversation_id": conv_id, "message": msg, "metadata": self.metadata.get(conv_id, {})}
                        )
                        break
            return results
        ...
        def export_to_json(self, filepath: str):
            """JSON으로 내보내기"""
            export_data = {"conversations": {}, "metadata": self.metadata, "export_date": datetime.now().isoformat()}

            for conv_id, messages in self.storage.items():
                export_data["conversations"][conv_id] = [{"role": msg.role, "content": msg.content} for msg in messages]
        ...
        def import_from_json(self, filepath: str):
            ...
            for conv_id, messages in data["conversations"].items():
                msg_objects = [Message(role=msg["role"], content=msg["content"]) for msg in messages]
    ...
    backup.save_with_metadata(
        "support_001",
        support_messages,
        {"customer_id": "CUST123", "issue_type": "technical", "priority": "high", "status": "ongoing"},
    )
    ...
    learning_messages = [
        ...
        Message(
            role="assistant",
            content="파이썬을 배우기 시작하신다니 훌륭한 선택입니다! 초보자를 위한 단계별 학습 방법을 안내해드리겠습니다.",
        ),
    ...
    backup.save_with_metadata(
        "learning_001", learning_messages, {"topic": "python", "level": "beginner", "session_type": "consultation"}
    )
...
def example_tool_interaction_backup():
    ...
    tools = [
        {"type": "function", "function": {"name": "get_time", "description": "현재 시간을 가져옵니다"}},
        {
            "type": "function",
            "function": {
                "name": "calculate",
    ...
    questions = ["현재 시간이 몇 시인가요?", "1234 * 5678을 계산해주세요.", "방금 계산한 결과를 1000으로 나누면?"]
    ...
    for question in questions:
        ...
        # 도구와 함께 응답
        reply = llm.ask_with_tools(question, tools=tools, tool_functions={"get_time": get_time, "calculate": calculate})
        ...
        ai_msg = Message(role="assistant", content=reply.text)
        # tool_calls 속성이 있으면 설정
        if hasattr(reply, "tool_calls") and reply.tool_calls:
            ai_msg.tool_calls = reply.tool_calls
        messages.append(ai_msg)
