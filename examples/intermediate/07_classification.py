...
"""
        ...
        try:
            ...
            print(f"추가 정보: 긴급도={info.get('urgency', 'N/A')}, "
...
def example_topic_classification():
    ...
    for headline in headlines:
        # 단일 주제 분류
        reply = llm.ask(
            f"다음 뉴스 헤드라인의 주제를 분류하세요: '{headline}'",
            choices=topics
        )
...
def example_multi_label_classification():
    ...
    # 가능한 장르들
    all_genres = [
        "액션", "드라마", "코미디", "로맨스", "SF", "판타지", 
        "스릴러", "공포", "뮤지컬", "다큐멘터리", "가족", "모험"
    ]
